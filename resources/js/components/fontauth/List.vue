  ...
  <div class="mb-5">
    ...
    <!-- modal end -->
      ...
      <div class="d-flex justify-content-between mb-5">
        ...
        <button @click="$router.push({path: '/user/profile'})" class="btn btn-add-cart">Profile</button>
      </div>
        ...
        <div class="row">
          ...
          <div class="col-md-4 d-flex align-items-center justify-content-between">
            <div>
              ...
              <p v-else-if="order.status == 1" class="m-0 text-primary">Status: Collecting</p>
              <p v-else-if="order.status == 2" class="m-0 text-warning">Status: Outing</p>
              <p v-else-if="order.status == 3" class="m-0 text-danger">Status: Failed</p>
              <p v-else class="m-0 text-success">Status: Complete</p>
              <h6>{{order.subtotal}}</h6>
            </div>
            <button @click="viewOrder(order.id)" class="btn btn-add-cart">View</button>
          </div>
        </div>
      </div>
